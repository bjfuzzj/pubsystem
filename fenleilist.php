...
if($fenlei_flag) $fenlei_nav_str = get_fenlei_nav($t_name, $parent_id);
...
while($row = mysql_fetch_array($res))
{
	...
	$del_str = sprintf("<a href='delfenlei.php?d_id=%s&t_id=%s&p_id=%s&parent_id=%s' onclick='return del_action();'>删除</a>", $d_id, $t_id, $p_id, $parent_id);
	$update_str = sprintf("<a href='editfenlei.php?d_id=%s&t_id=%s&p_id=%s'>修改</a>", $d_id, $t_id, $p_id);
...
function get_fenlei_nav($t_name, $id)
{
	global $proj_mysql;
	global $p_id, $t_id;
	...
	$fenlei_data = array();
	$sqlstr = "select id, pid, cname  from $t_name";
	$res = mysql_query($sqlstr, $proj_mysql) or exit(mysql_error() . "\n" . $sqlstr);
	while($row = mysql_fetch_array($res))
	{
		$fenlei_data["$row[id]"] = $row;
	}


	$nav_str = $fenlei_data[$id][cname];
	$pid = $fenlei_data[$id][pid];

	while($pid != "" && $pid != "0")
	{
		$row = $fenlei_data[$pid];

		$tstr = sprintf('<a href=fenleilist.php?p_id=%s&t_id=%s&parent_id=%s>%s</a>', $p_id, $t_id, $row[id], $row[cname]);
		$nav_str = $tstr . " &gt; " . $nav_str;
		$pid = $row[pid];
	}

	$tstr = sprintf('<a href=fenleilist.php?p_id=%s&t_id=%s&parent_id=0>根分类</a>', $p_id, $t_id);
...
<tr>
<td><?php echo $fenlei_nav_str ?> </td>
<td align=right>
<input name=cname>
<input type=button  value="添加分类" onclick="add_fenlei(this.form)">
</td>
</tr>
</table>
<script>
document.myform.cname.focus();
function add_fenlei(tform)
{
	tform.action='addfenlei.php?<?php echo "t_id=$t_id&p_id=$p_id&parent_id=$parent_id" ?>';
	tform.submit();
	return;
}
</script>
